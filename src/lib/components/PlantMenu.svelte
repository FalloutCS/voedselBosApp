...
>
    ...
    <div class="mb-4">
        ...
        >
            <svg
                ...
                stroke-linecap="round"
                stroke-linejoin="round"
                class="transition-transform duration-300 {showFilters
                    ? 'rotate-180'
            ...
            {#if activeFilterCount > 0}
                <span
                    class="bg-violet-600 text-white text-xs px-2 py-0.5 rounded-full ml-1"
                    >{activeFilterCount} actief</span
                >
            {/if}
        </button>
    </div>
    ...
    {#if showFilters}
        <div transition:slide={{ duration: 300, axis: "y" }}>
            <div
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4 p-4 bg-violet-50/80 rounded-xl border border-violet-100"
            >
                {#each activeFilters as filterDef (filterDef.key)}
                    <div class="flex flex-col gap-1">
                        <label
                            for={filterDef.key}
                            class="text-xs font-bold text-violet-800 uppercase tracking-wide ml-1"
                ...
                {#if activeFilterCount > 0}
                    <div
                        class="col-span-full flex justify-end mt-2 pt-2 border-t border-violet-200/50"
                    >
                        <button
                            type="button"
    ...
    <div class="text-xs text-gray-400 mb-2 px-1">
        {filteredPlants.length} resultaten gevonden
    </div>

    <div
        class="grow overflow-y-auto pr-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"
