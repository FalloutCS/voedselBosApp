...
// --- Translation Helpers ---
  function translateSun(code?: string): string {
    switch(code) {
        case "N": return "Volle zon";   // Past bij "Lichtbehoefte"
  ...
  function translateWind(code?: string): string {
    switch(code) {
        case "W": return "Zeer bestendig"; // Past bij "Windbestendigheid"
        case "M": return "Matig";
        case "S": return "Kwetsbaar";      // Duidelijker dan 'gevoelig'
        default: return "Onbekend";
    }
  }
  ...
  function translateNitrogen(need?: number): string {
    ...
    if (need >= 3) return "Groot";      // Past bij "Stikstofbehoefte"
  ...
  <div class="relative grow border-2 border-emerald-100 rounded-lg overflow-hidden bg-white shadow-sm">
    ...
    {#if activeCellIndex !== null && activePlantData}
      ...
      <div
        ...
        class:border-emerald-500={currentIssues.length === 0}
        class:border-red-500={currentIssues.length > 0}
      ...
      >
        ...
        <div class="px-6 py-4 border-b border-stone-100 flex justify-between items-start bg-white z-10">
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <span class={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${currentIssues.length === 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                        {currentIssues.length === 0 ? 'Gezond' : `${currentIssues.length} Probleem`}
                    </span>
                </div>
        ...
        <div class="p-6 overflow-y-auto grow bg-stone-50/50">
            ...
            <div class="flex gap-4 mb-5 p-3 bg-white rounded-lg border border-stone-200 shadow-sm">
            ...
            <div class="space-y-2">
                {#if currentIssues.length > 0}
                    <h4 class="text-xs font-bold text-stone-500 uppercase tracking-wider mb-2 ml-1">Diagnose & Advies</h4>
